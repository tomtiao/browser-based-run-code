...
["_main","__emscripten_thread_init","__emscripten_thread_exit","__emscripten_thread_crashed","__emscripten_thread_mailbox_await","__emscripten_tls_init","_pthread_self","checkMailbox","establishStackSpace","invokeEntryPoint","PThread","_fflush","__emscripten_check_mailbox","onRuntimeInitialized"].forEach((prop) => {
...
var wasmBinaryFile;
if (Module['locateFile']) {
  wasmBinaryFile = 'test_pthread.wasm';
  if (!isDataURI(wasmBinaryFile)) {
    wasmBinaryFile = locateFile(wasmBinaryFile);
  }
} else {
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  wasmBinaryFile = new URL('test_pthread.wasm', import.meta.url).href;
}
...
// end include: preamble.js
  ...
  var PThread = {unusedWorkers:[],runningWorkers:[],tlsInitFunctions:[],pthreads:{},nextWorkerID:1,debugInit:function() {
        ...
        if (!Module['locateFile']) {
          worker = new Worker(new URL('test_pthread.worker.js', import.meta.url));
        } else {
        // Allow HTML module to configure the location where the 'worker.js' file will be loaded from,
        // via Module.locateFile() function. If not specified, then the default URL 'worker.js' relative
        // to the main html file is loaded.
        var pthreadMainJs = locateFile('test_pthread.worker.js');
